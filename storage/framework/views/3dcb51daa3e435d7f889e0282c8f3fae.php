...
<div class="max-w-7xl mx-auto px-4 py-12" x-data="{ tab: '<?php echo e($activeTab); ?>', search: '' }">
    
    
    <div class="bg-slate-900 rounded-t-[2.5rem] p-10 text-center shadow-lg border-b-4 border-amber-500">
        <h1 class="text-2xl font-black text-white uppercase italic tracking-tight"><?php echo e($title); ?></h1>
    ...
    <div class="bg-white rounded-b-[2.5rem] shadow-sm border border-slate-100 p-8 min-h-[600px]">
            ...
            <?php else: ?>
                <div class="overflow-x-auto border border-slate-200 rounded-xl">
                    <table class="w-full text-left border-collapse bg-white">
                        ...
                        <tbody>
                            <?php $__currentLoopData = $semuaData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $doc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="hover:bg-slate-50 border-b border-slate-100" x-show="'<?php echo e(strtolower($doc->title . ' ' . $doc->opd_name)); ?>'.includes(search.toLowerCase())">
                                    <td class="p-4 text-sm font-bold text-center italic text-slate-400"><?php echo e($index + 1); ?></td>
                                    <td class="p-4 text-[11px] uppercase font-bold text-slate-600 leading-tight"><?php echo e($doc->opd_name); ?></td>
                                    <td class="p-4">
                                    ...
                                    <td class="p-4">
                                        <a href="<?php echo e(route('public.informasi.show', $doc->id)); ?>" class="text-blue-600 font-extrabold hover:underline text-sm"><?php echo e($doc->title); ?></a>
                                    </td>
                                    <td class="p-4 text-center">
                                        <a href="<?php echo e(route('public.informasi.show', $doc->id)); ?>" class="inline-flex items-center justify-center w-10 h-10 bg-cyan-50 text-cyan-600 rounded-full hover:bg-cyan-600 hover:text-white transition-all">
        ...
        <?php else: ?>
            ...
            <div class="flex flex-wrap justify-center gap-3 mb-10">
                <?php $__currentLoopData = ['berkala' => '📅 Berkala', 'setiap saat' => '🔓 Setiap Saat', 'serta merta' => '🚨 Serta Merta', 'dikecualikan' => '🛡️ Dikecualikan']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button @click="tab = '<?php echo e($key); ?>'" 
                            :class="tab === '<?php echo e($key); ?>' ? '<?php echo e($key == 'berkala' ? 'bg-blue-600' : ($key == 'setiap saat' ? 'bg-emerald-600' : ($key == 'serta merta' ? 'bg-red-600' : 'bg-slate-900'))); ?> text-white shadow-lg' : 'bg-slate-100 text-slate-600'" 
            ...
            <?php $__currentLoopData = ['berkala', 'setiap saat', 'serta merta', 'dikecualikan']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div x-show="tab === '<?php echo e($kat); ?>'" x-cloak>
                    
                    
                    <?php if($kat === 'dikecualikan'): ?>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $filtered; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idKel => $items): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="mb-12">
                            <div class="flex items-center gap-4 mb-6">
                                <span class="bg-amber-100 text-amber-700 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border border-amber-200">
                                    📁 <?php echo e($items->first()->kategori_kelompok->name ?? 'Informasi Lainnya'); ?>
                            ...
                            <div class="grid gap-4">
                                <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $doc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="flex flex-col md:flex-row md:items-center justify-between p-6 bg-white border border-slate-100 rounded-[2rem] hover:shadow-lg transition-all group"
                                         x-show="'<?php echo e(strtolower($doc->title . ' ' . $doc->opd_name)); ?>'.includes(search.toLowerCase())">
                                        <div class="flex-1">
                                            <h4 class="text-slate-800 font-bold text-sm group-hover:text-blue-600 transition-colors"><?php echo e($doc->title); ?></h4>
                                            <span class="px-2 py-0.5 bg-slate-50 text-[9px] text-slate-500 font-black uppercase rounded-md italic mt-2 inline-block border border-slate-100">🏛️ <?php echo e($doc->opd_name); ?></span>
                                        </div>
                                        <a href="<?php echo e(route('public.informasi.show', $doc->id)); ?>" class="mt-4 md:mt-0 px-6 py-2 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase hover:bg-blue-600 transition-all text-center">Lihat Detail</a>
                                    </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        ...
                        <?php if($kat !== 'dikecualikan'): ?>
                            <div class="py-20 text-center text-slate-400 italic text-xs uppercase font-bold tracking-widest">Data <?php echo e($kat); ?> Belum Tersedia</div>
                        <?php endif; ?>
                    <?php endif; ?>
                </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.public', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/public/informasi/index.blade.php ENDPATH**/ ?>