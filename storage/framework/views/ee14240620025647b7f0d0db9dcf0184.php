        ...
        <div class="flex flex-wrap md:flex-nowrap items-end gap-3 bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm">
            ...
            <div class="flex flex-col gap-1">
                ...
                <select id="filterJenis" class="px-4 py-2 bg-slate-50 border-none rounded-xl text-xs font-bold focus:ring-2 focus:ring-blue-500 w-48">
                    <option value="permohonan" <?php echo e(request('jenis') == 'permohonan' ? 'selected' : ''); ?>>Permohonan Informasi</option>
                    <option value="keberatan" <?php echo e(request('jenis') == 'keberatan' ? 'selected' : ''); ?>>Keberatan Informasi</option>
                </select>
            </div>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden transition-all">
        <div class="p-6 border-b border-slate-50 bg-slate-50/30">
            <h3 class="font-black text-slate-800 uppercase text-xs tracking-widest flex items-center gap-2">
                ...
                Data <?php echo e(request('jenis') == 'keberatan' ? 'Keberatan' : 'Permohonan'); ?> - <?php echo e(request('tahun', date('Y'))); ?>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr>
                        ...
                        <?php if(request('jenis') == 'keberatan'): ?>
                ...
                <tbody class="divide-y divide-slate-50">
                    <?php $__empty_1 = true; $__currentLoopData = $permohonans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        <td class="px-6 py-4 text-xs font-bold text-slate-400"><?php echo e($index + 1); ?></td>
                        ...
                        <td class="px-6 py-4">
                            <div class="text-xs font-black text-slate-900">
                                <?php echo e(request('jenis') == 'keberatan' ? ($item->permohonan->nama ?? 'N/A') : $item->nama); ?>

                            </div>
                            <div class="text-[9px] text-slate-400 font-bold uppercase tracking-tighter">
                                <?php echo e(request('jenis') == 'keberatan' ? $item->nomor_registrasi_keberatan : $item->nomor_registrasi); ?>

                            </div>
                        </td>
                        <td class="px-6 py-4 text-xs text-slate-600 leading-relaxed">
                            <?php if(request('jenis') == 'keberatan'): ?>
                                <span class="bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-black text-[9px]">KODE <?php echo e($item->alasan_kode); ?></span>
                                <span class="ml-1 italic text-slate-400"><?php echo e(Str::limit($item->kronologi, 40)); ?></span>
                            <?php else: ?>
                                <?php echo e(Str::limit($item->rincian_informasi, 60)); ?>
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            ?>
                            <span class="px-3 py-1 border <?php echo e($color); ?> rounded-full text-[9px] font-black uppercase tracking-widest">
                                <?php echo e($status); ?>
...
<script>
    ...
    function printLaporan() {
        ...
        frame.src = `<?php echo e(route('admin.laporan.cetak')); ?>?jenis=${jenis}&tahun=${tahun}`;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/admin/laporan/index.blade.php ENDPATH**/ ?>