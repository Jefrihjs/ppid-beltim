...
<body>
    ...
    <table style="margin-left: 20px;">
        ...
        <tr><td>Nomor Telp./Email</td><td>: {{ $permohonan->no_hp }} / {{ $permohonan->email }}</td></tr>
    </table>
    ...
    <table class="table-isi">
        <tr>
            ...
            <td>
                {{-- Sesuaikan: Di Heidi namanya 'penguasaan', bukan 'penguasaan_informasi' --}}
                @if(($pemberitahuan->penguasaan ?? '') == 'opd_lain')
                    <strong>{{ $pemberitahuan->nama_opd ?? 'Badan Publik Lain' }}</strong>
                @else
        ...
        <tr>
            ...
            <td>{{ $pemberitahuan->bentuk_fisik ?? '-' }}</td>
        </tr>
        <tr>
            <td class="text-center">3</td>
            <td>Biaya yang dibutuhkan</td>
            <td>
                @if(($pemberitahuan->total_biaya ?? 0) > 0)
                    Rp {{ number_format($pemberitahuan->total_biaya, 0, ',', '.') }}
                @else
                    Gratis
                @endif
            </td>
    ...
    <table class="footer-table">
        <tr>
            <td width="30%">
                {{-- QR CODE DISINI --}}
                <img src="data:image/png;base64,{{ base64_encode(QrCode::format('png')->size(100)->generate(url('/cek-permohonan/'.$permohonan->kode_tracking))) }}">
            </td>
    ...
    <div style="position: fixed; bottom: 0; width: 100%; text-align: center; font-size: 8px; font-style: italic; border-top: 1px solid #ccc; padding-top: 5px;">
        *Berkas ini dicetak pada hari {{ now()->translatedFormat('l, d F Y') }} melalui aplikasi PPID - Lawang Beltim
    </div>
</body>
</html>