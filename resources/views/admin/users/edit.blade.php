        ...
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" required class="w-full rounded-xl border-slate-200 text-sm">
                </div>

                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Email Dinas</label>
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="w-full rounded-xl border-slate-200 text-sm">
                </div>
                ...
                <div>
                    ...
                    <select name="role" required class="w-full rounded-xl border-slate-200 text-sm">
                        <option value="admin" {{ $user->role == 'admin' ? 'selected' : '' }}>Admin (Layanan PPID)</option>
                        <option value="superadmin" {{ $user->role == 'superadmin' ? 'selected' : '' }}>Superadmin (Akses Penuh)</option>
                    </select>
                </div>
