...
<div class="space-y-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col justify-between">
            ...
            <div class="my-2">
                <h3 class="text-4xl font-black text-slate-900 leading-none">{{ $stats['total_informasi'] ?? 0 }}</h3>
            </div>
            <div class="text-[10px] font-black text-blue-600 uppercase tracking-widest">Total Dokumen</div>
        </div>

        {{-- Permohonan Baru --}}
        <a href="{{ route('admin.permohonan.index') }}" class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col justify-between hover:border-amber-500 transition-all">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Permohonan Baru</p>
            <div class="my-2">
                <h3 class="text-4xl font-black text-amber-500 leading-none">{{ $stats['permohonan_baru'] ?? 0 }}</h3>
            </div>
            <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Belum Diproses</div>
        </a>

        {{-- Pesan Masuk --}}
        <a href="{{ route('admin.pesan.index') }}" class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col justify-between hover:border-slate-900 transition-all">
            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Pesan Masuk</p>
            <div class="my-2">
                <h3 class="text-4xl font-black text-slate-900 leading-none">{{ $stats['pesan_masuk'] ?? 0 }}</h3>
            </div>
        ...
        <div class="bg-slate-900 p-6 rounded-[2rem] shadow-xl shadow-slate-200 flex flex-col justify-between relative overflow-hidden">
            ...
            <div class="my-2">
                <h3 class="text-4xl font-black text-white leading-none">{{ number_format($totalVisitors ?? 0) }}</h3>
            </div>
            <div class="text-[10px] font-black text-emerald-400 uppercase tracking-widest italic">All Time Records</div>
        </div>
    </div>

    {{-- Grafik Pengunjung & Visitor Hari Ini --}}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-white border border-slate-100 p-10 rounded-[3rem] shadow-sm flex flex-col justify-center items-center text-center relative overflow-hidden group">
            ...
            <div class="relative z-10">
                ...
                <h4 class="text-7xl font-black text-slate-900 mb-2">{{ $todayVisitors ?? 0 }}</h4>
                <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">People Online</p>
            </div>
        </div>
    </div>

    {{-- Tabel Ringkasan (Tetap di Bawah) --}}
    <div class="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
            ...
            <div class="flex gap-4 items-center">
                {{-- TOMBOL CETAK --}}
                <a href="{{ route('admin.permohonan.cetak') }}" target="_blank" class="flex items-center gap-2 bg-emerald-100 text-emerald-700 px-4 py-2 rounded-xl text-[10px] font-black hover:bg-emerald-600 hover:text-white transition uppercase tracking-widest">
                    {{-- SVG DENGAN UKURAN TERKUNCI --}}
                ...
                <a href="{{ route('admin.permohonan.index') }}" class="text-[10px] font-black text-blue-600 hover:text-blue-800 transition uppercase tracking-widest">Lihat Semua Data</a>
            </div>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-50">
                    @forelse($recent_permohonan as $p)
                    <tr class="hover:bg-slate-50/80 transition group">
                        <td class="px-10 py-7">
                            <span class="font-black text-slate-800 text-sm block">{{ $p->nama }}</span>
                            <span class="text-[10px] font-bold text-slate-400 tracking-wider">{{ $p->nik }}</span>
                        </td>
                        <td class="px-10 py-7 text-xs text-slate-500 max-w-xs truncate font-medium">{{ $p->tujuan_penggunaan }}</td>
                        <td class="px-10 py-7">
                            @php
                                $statusClasses = [
                            ...
                            @endphp
                            <span class="px-4 py-1.5 {{ $class }} rounded-full text-[10px] font-black uppercase tracking-tighter">
                                {{ $p->status }}
                            </span>
                        </td>
                        <td class="px-10 py-7 text-right">
                            <a href="{{ route('admin.permohonan.show', $p->id) }}" class="inline-flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-xl text-[10px] font-black hover:bg-blue-600 transition shadow-sm uppercase tracking-widest">
                                Detail
                            </a>
                        </td>
                    </tr>
