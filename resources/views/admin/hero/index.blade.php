...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <a href="{{ route('admin.hero.create') }}" class="inline-flex items-center justify-center px-6 py-3 bg-slate-900 text-white rounded-2xl font-black hover:bg-blue-600 transition-all shadow-xl shadow-slate-200 active:scale-95 text-sm uppercase tracking-widest">
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-all">
                        <td class="py-6 px-6">
                            <div class="relative w-48 h-28 overflow-hidden rounded-2xl shadow-sm border-2 border-white group-hover:shadow-md transition-all">
                                <img src="{{ asset('storage/images/' . $hero->image) }}" class="w-full h-full object-cover">
                            </div>
                        </td>
                        <td class="py-6 px-6">
                            <div class="font-black text-slate-800 text-lg leading-tight mb-1">{!! $hero->title !!}</div>
                            <div class="text-sm text-slate-500 font-medium italic line-clamp-2 max-w-md">{{ $hero->subtitle }}</div>
                        </td>
                        ...
                        <td class="py-6 px-6">
                            <div class="flex justify-center gap-3">
                                {{-- Tombol Edit --}}
                                <a href="{{ route('admin.hero.edit', $hero->id) }}" class="p-3 bg-slate-100 text-slate-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm group/btn">
                                ...
                                {{-- Tombol Hapus --}}
                                <form action="{{ route('admin.hero.destroy', $hero->id) }}" method="POST" onsubmit="return confirm('Apakah Bapak yakin ingin menghapus slide ini?')">
                                    @csrf @method('DELETE')
