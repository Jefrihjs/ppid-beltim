...
<footer class="bg-slate-950 text-slate-400">
    <div class="max-w-7xl mx-auto px-6 py-20">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
            ...
            <div class="space-y-6">
                <div class="flex items-center gap-3">
                    <img src="{{ asset('images/logo-ppid-beltim.png') }}" class="h-12 w-auto brightness-0 invert">
            ...
            <div>
                ...
                <ul class="space-y-3 text-sm">
                    {{-- 1. Profil PPID --}}
                    <li>
                        <a href="{{ route('profil.tentang') }}" class="hover:text-amber-500 transition-colors flex items-center gap-2">
                    ...
                    {{-- 2. Alur Permohonan --}}
                    <li>
                        <a href="{{ route('permohonan.informasi') }}" class="hover:text-amber-500 transition-colors flex items-center gap-2">
                            <span>&rsaquo;</span> Alur Permohonan
                        </a>
                    </li>

                    {{-- 3. Informasi Publik (Arahkan ke DIP Utama) --}}
                    <li>
                        <a href="{{ route('informasi.utama') }}" class="hover:text-amber-500 transition-colors flex items-center gap-2">
                            <span>&rsaquo;</span> Informasi Publik
                        </a>
                    </li>

                    {{-- 4. Regulasi & Hukum (Arahkan ke Syarat & Ketentuan) --}}
                    <li>
                        <a href="{{ route('terms.conditions') }}" class="hover:text-amber-500 transition-colors flex items-center gap-2">
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3 mb-8">
                    ...
                    <a href="{{ $soc['link'] }}" class="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-800 border border-slate-700 text-slate-300 hover:bg-amber-500 hover:text-slate-900 transition-all duration-300 shadow-lg">
                        @if($soc['icon'] == 'facebook')
                ...
                <div class="space-y-3 pt-4 border-t border-slate-900">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-xs font-black text-slate-300 tracking-tighter">{{ number_format($todayVisitors ?? 0) }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-[9px] font-black text-slate-500 uppercase tracking-widest">Total Hits</span>
                        <span class="text-xs font-black text-slate-300 tracking-tighter">{{ number_format($totalVisitors ?? 0) }}</span>
                    </div>
        ...
        <div class="border-t border-slate-800 mt-16 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500">
            <p>&copy; {{ date('Y') }} PEMKAB BELITUNG TIMUR. ALL RIGHTS RESERVED.</p>
            <div class="flex gap-6">
                <a href="{{ route('privacy.policy') }}" class="hover:text-amber-500 transition-colors">Kebijakan Privasi</a>
                <a href="{{ route('terms.conditions') }}" class="hover:text-amber-500 transition-colors">Syarat & Ketentuan</a>
            </div>
        </div>
