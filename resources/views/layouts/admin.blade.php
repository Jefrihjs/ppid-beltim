    ...
    <div class="flex flex-1 bg-slate-100">
        ...
        <aside class="w-64 bg-white border-r border-slate-200 sticky top-[68px] h-[calc(100vh-68px)] p-6 hidden md:block overflow-y-auto custom-scrollbar">
            ...
            <nav class="space-y-2">
                ...
                {{-- 2. LAYANAN PPID (DROPDOWN) --}}
                <div x-data="{ openPPID: {{ request()->routeIs('admin.permohonan.*', 'admin.keberatan.*', 'admin.laporan.*') ? 'true' : 'false' }} }" class="space-y-1">
                    <button @click="openPPID = !openPPID" 
                            class="w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all {{ request()->routeIs('admin.permohonan.*', 'admin.keberatan.*', 'admin.laporan.*') ? 'bg-slate-100 text-slate-900' : 'text-slate-600 hover:bg-slate-50' }}">
                    ...
                    <div x-show="openPPID" x-cloak class="pl-4 space-y-1 mt-1">
                        <a href="{{ route('admin.permohonan.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition {{ request()->routeIs('admin.permohonan.*') ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('admin.permohonan.*') ? 'bg-blue-400' : 'bg-slate-300' }}"></div>
                            <span class="text-[11px] font-bold">Permohonan Informasi</span>
                        </a>
                        <a href="{{ route('admin.keberatan.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition {{ request()->routeIs('admin.keberatan.*') ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('admin.keberatan.*') ? 'bg-red-400' : 'bg-slate-300' }}"></div>
                            <span class="text-[11px] font-bold">Keberatan Informasi</span>
                        </a>
                        <a href="{{ route('admin.laporan.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition {{ request()->routeIs('admin.laporan.*') ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}">
                            <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('admin.laporan.*') ? 'bg-emerald-400' : 'bg-slate-300' }}"></div>
                ...
                <a href="{{ route('admin.informasi.index') }}" 
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.informasi.*') ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-600 hover:bg-slate-50' }}">
                ...
                {{-- 4. SUB-JUDUL (KELOMPOK) --}}
                <a href="{{ route('admin.categories.index') }}" 
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.categories.*') ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-600 hover:bg-slate-50' }}">
                ...
                <a href="{{ route('admin.opd.index') }}" 
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.opd.*') ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-600 hover:bg-slate-50' }}">
                    ...
                    <div class="flex items-center justify-between w-full">
                        ...
                        <span class="text-[9px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full {{ request()->routeIs('admin.opd.*') ? 'bg-slate-800 text-white' : '' }}">41</span>
                    </div>
                </a>
                ...
                <a href="{{ route('admin.gallery.index') }}" 
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.gallery.*', 'admin.video.*') ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-600 hover:bg-slate-50' }}">
                ...
                <a href="{{ route('admin.visitors.index') }}" 
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.visitors.*') ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-600 hover:bg-slate-50' }}">
            ...
            <div class="mt-10 pt-6 border-t border-slate-100 pb-10">
                ...
                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition {{ request()->routeIs('profile.edit') ? 'bg-slate-900 text-white shadow-lg shadow-slate-200' : 'text-slate-600 hover:bg-slate-50' }}">
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            @yield('content') {{-- VARIABEL KHUSUS SEPERTI $GALLERIES AKAN MASUK KE SINI --}}
        </main>
    </div>
